...
var options4 = {
  ...
  $.getJSON('../data/health-data-king-county.json', function(data) {
    $.each(data, function(key, val) {
      ...
      if (val.Gender == 'Male') {
        ...
        if (val.Calculated_Age > 60) {
            mo.push(val.Calculated_BMI_Dec);
            mOld++;
            mOldTot = mOldTot + val.Calculated_BMI;
        } else if (val.Calculated_Age > 40) {
            mm.push(val.Calculated_BMI_Dec);
            mMed++;
            mMedTot = mMedTot + val.Calculated_BMI;
        } else if (val.Calculated_Age > 20) {
            my.push(val.Calculated_BMI_Dec);
            mYoung++;
            mYoungTot = mYoungTot + val.Calculated_BMI;
        } else {
        	mc.push(val.Calculated_BMI_Dec);
            mChild++;
            mChildTot = mChildTot + val.Calculated_BMI;
        }

        if (val.Ethnicity == 'African American') {
            ma.push(val.Calculated_BMI_Dec);
        } else if (val.Ethnicity == 'Hispanic') {
            mh.push(val.Calculated_BMI_Dec);
        } else if (val.Ethnicity == 'White') {
            mw.push(val.Calculated_BMI_Dec);
        } else {
            mn.push(val.Calculated_BMI_Dec);
        }        
        ...
        if (val.Calculated_Age > 60) {
            fo.push(val.Calculated_BMI_Dec);
            fOld++;
            fOldTot = fOldTot + val.Calculated_BMI;
        } else if (val.Calculated_Age > 40) {
            fm.push(val.Calculated_BMI_Dec);
            fMed++;
            fMedTot = fMedTot + val.Calculated_BMI;
        } else if (val.Calculated_Age > 20) {
			fy.push(val.Calculated_BMI_Dec);
            fYoung++;
            fYoungTot = fYoungTot + val.Calculated_BMI;
        } else {
        	fc.push(val.Calculated_BMI_Dec);
            fChild++;
            fChildTot = fChildTot + val.Calculated_BMI;
        }

        if (val.Ethnicity == 'African American') {
            fa.push(val.Calculated_BMI_Dec);
        } else if (val.Ethnicity == 'Hispanic') {
            fh.push(val.Calculated_BMI_Dec);
        } else if (val.Ethnicity == 'White') {
            fw.push(val.Calculated_BMI_Dec);
        } else {
            fn.push(val.Calculated_BMI_Dec);
        }   
