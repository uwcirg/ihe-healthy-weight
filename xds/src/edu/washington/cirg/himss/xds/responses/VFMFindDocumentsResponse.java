...
/**
 * Class represents a VFMFindDocuments query response
 * returned from a VFM registry, in response to a VFMFindDocuments request
 * 
...
public class VFMFindDocumentsResponse extends RegistryResponse {

	private static Log log = LogFactory.getLog(VFMFindDocumentsResponse.class);
	...
	public VFMFindDocumentsResponse(SOAPMessage response)
		...
		while (iterator.hasNext()) {
			...
			if (nodeList.size() < 1) {
				...
				while (nodeIterator.hasNext()) {
					...
					docUri = docUri.concat(node.getText());
					if (nodeIterator.hasNext()) {
